...
pub struct MediaItem {
    ...
    pub poster_path: Option<String>,   // local cached path
    pub backdrop_path: Option<String>, // local cached path
...
fn parse_filename(filename: &str) -> ParsedFilename {
    ...
    // Extract season and episode: S01E09, s01e09
    ...
    // Extract year: (2024) or .2024. but not part of resolution
    ...
    // Extract title: everything before S01E09, year, or quality marker
    ...
    // Clean title: replace dots, underscores, hyphens with spaces
...
fn scan_folders(folder_paths: Vec<String>) -> Result<Vec<MediaItem>, String> {
    ...
    for folder in &folder_paths {
        ...
        for entry in entries.flatten() {
            ...
            // Check if this folder contains season subfolders or video files
            ...
            // If there are direct video files with SxxExx patterns but no season folders,
            // group them as episodes
            if !has_season_folders && !direct_videos.is_empty() {
                ...
                if !episode_videos.is_empty() {
                    ...
                    has_season_folders = true; // Treat as series
                }
            ...
            // Parse folder name for TMDB search
        ...
        // Process loose videos at the root of this watched folder
        ...
        // Add grouped series from loose files
    ...
    // Sort: series first, then by name
...
async fn fetch_all_metadata(
    ...
    for item in &items {
        ...
        // Extract the original folder name from path
        ...
        .await
        {
            ...
            Err(_) => {
                // Skip items that fail — don't break the whole batch
                continue;
            }
        }

        // Small delay to respect TMDB rate limits (40 req / 10 sec)
...
async fn play_media(path: String) -> Result<(), String> {
    // Use the system's default application to open the video file
